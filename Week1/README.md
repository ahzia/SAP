# Week 1: Understanding Technology - Exercise Guide

## 📚 Overview

This week covers **Understanding Technology** with a focus on:
- Cloud Computing
- Technology Evolution
- Artificial Intelligence
- Robotics & Automation
- SAP Integration

---

## 📅 Daily Schedule

### Day 1 (Monday, 17 Feb) - Welcome & Introduction
**Topics Covered:**
- What is Technology?
- Technology Terminology (Digital Ledger, Computing, SaaS, IoT, Network Node)
- Evolution of Apps, Devices and Software Used
- Blockchain Basics

**Exercises:**
- [📄 01_excersize.md - Virtual Tech Bingo (Icebreaker)](Day1/01_excersize.md)
- [📄 02_excersize.md - Understanding Technology Before SAP](Day1/02_excersize.md)
- [📄 03_excersize.md - What is Technology? & Terminology](Day1/03_excersize.md)
- [📄 04_excersize.md - Blockchain Basics](Day1/04_excersize.md)

---

### Day 2 (Tuesday, 18 Feb) - Cloud Computing & SAP Cloud
**Topics Covered:**
- What is Cloud?
- History of Cloud
- What does Cloud do? (Storage, Computing Power, SaaS, Disaster Recovery)
- Key Characteristics of Cloud Computing
- Key Components of Cloud Computing
- Examples of Cloud Services
- Practical Aspects of Cloud (Scalability, Cost-Effectiveness, Accessibility, Innovation)
- SAP Cloud
- Modules in SAP Cloud
- How Cloud Combines with Other Tech Tools
- How SAP Cloud Integrates with Other Tools

**Exercises:**
- [📄 01_excersize.md - Cloud Computing Fundamentals](Day2/01_excersize.md)
- [📄 02_excersize.md - SAP Cloud & Integration](Day2/02_excersize.md)
- [📄 03_excersize.md - Group Exercise: Evolution of Data Storage & IoT Smart City](Day2/03_excersize.md)

---

### Day 3 (Wednesday, 19 Feb) - Transition to AI
**Topics Covered:**
- SAP BTP and Cloud Computing (Video Learning)
- System Software vs. Application Software (Video Learning)
- Artificial Intelligence Introduction
- AI Key Components (Machine Learning, NLP, Computer Vision, Robotics, Expert Systems)
- AI Functionalities (Predictive Analytics, Speech Recognition, Image Recognition, Recommendation Systems, Autonomous Systems)
- How AI Works (5 Steps)
- How AI Combines with Other Technologies (IoT, Cloud, Big Data, Blockchain)

**Exercises:**
- [📄 01_excersize.md - Video Learning: SAP BTP & Cloud Computing](Day3/01_excersize.md)
- [📄 02_excersize.md - Video Learning: System Software vs. Application Software](Day3/02_excersize.md)
- [📄 03_excersize.md - Introduction to Artificial Intelligence](Day3/03_excersize.md)
- [📄 04_excersize.md - Afternoon Session: How AI Works & AI-Cloud Integration](Day3/04_excersize.md)

---

### Day 4 (Thursday, 20 Feb) - AI History, SAP AI & Introduction to RPA
**Topics Covered:**
- AI History and Evolution
- AI Advanced Concepts (Deep Learning, Neural Networks, Reinforcement Learning, GANs)
- How AI Combines with Other Technologies (IoT, Cloud, Big Data, Blockchain)
- SAP and AI
- SAP AI Functionalities (SAP Leonardo, SAP Conversational AI, SAP Predictive Analytics)
- SAP Cloud AI Functionalities
- Robotic Process Automation (RPA)
- Digital Innovation and Business Transformation

**Exercises:**
- [📄 01_excersize.md - AI History & Advanced Concepts](Day4/01_excersize.md)
- [📄 02_excersize.md - AI Integration & SAP AI](Day4/02_excersize.md)
- [📄 03_excersize.md - Video Learning: Robotic Process Automation (RPA)](Day4/03_excersize.md)
- [📄 04_excersize.md - Afternoon Session: SAP AI & Digital Innovation](Day4/04_excersize.md)

---

### Day 5 (Friday, 21 Feb) - Automation & Week 1 Conclusion
**Topics Covered:**
- Difference between Automation and AI
- Key Aspects and Components of Automation
- What is Automation?
- Key Components
- How is Automation Done?
- Automation in SAP Cloud
- SAP Automation (SAP Intelligent RPA, SAP Workflow Management, SAP AI Business Services)
- Automated Invoice Process in SAP
- Automated Billing in SAP
- Automated Allocation of Payments in SAP
- SAP Integration with External Systems
- Week 1 Conclusions and Recap

**Exercises:**
- [📄 01_excersize.md - Robotics & Automation](Day5/01_excersize.md)
- [📄 02_excersize.md - SAP Automation & Week 1 Wrap-Up](Day5/02_excersize.md)
- [📄 03_excersize.md - Afternoon Session: SAP Integration & Automation](Day5/03_excersize.md)

---

## 🎯 Exercise Types

Each day includes **2 exercises** designed for beginners:

1. **Conceptual Understanding** - Simple explanations, everyday examples, and basic concepts
2. **Interactive Activities** - Fun games, matching exercises, scavenger hunts, and hands-on learning

**Note:** Exercises are designed for students with no prior SAP knowledge - we focus on understanding concepts first!

---

## 💡 Tips for Students

- **Complete exercises during the online session** - They're designed for interactive learning
- **No prior knowledge needed** - Exercises start with simple, everyday examples
- **Work in groups** - Many exercises include discussion components
- **Take notes** - Use the notes sections provided in each exercise
- **Ask questions** - Exercises include reflection questions to guide your learning
- **Have fun!** - Exercises include games, scavenger hunts, matching activities, and creative exercises

---

## 📝 Exercise Format

Each exercise includes:
- ✅ Clear learning objectives
- ✅ Step-by-step activities
- ✅ Reflection questions
- ✅ Self-check quizzes
- ✅ Real-world scenarios
- ✅ SAP-specific examples
- ✅ Notes sections

---

## 🎉 Week 1 Learning Outcomes

By the end of this week, you should be able to:

- ✅ Understand cloud computing concepts and SAP Cloud
- ✅ Explain technology evolution from past to present
- ✅ Describe AI and its applications in SAP
- ✅ Differentiate between Robotics, Automation, and AI
- ✅ Identify SAP automation features and use cases
- ✅ Understand how technologies integrate with SAP

---

## 📚 Resources

- **Slides:** Source files available from your instructor
  - Understanding Technology 1.pptx - Week 1, Days 1-3
  - Understanding Technology 2.pptx - Week 1, Days 4-5
- **Course Notes:** Available from your instructor

---

## 🚀 Next Steps

After completing Week 1, you'll move on to:
- **Week 2:** Understanding Technology - Workshop on Cloud Computing
- **Week 2-6:** SAP Technology Consultant Professional Certificate

---

**Good luck and enjoy your learning journey!** 🌟

